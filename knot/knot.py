...
class Knot():
    def __init__(self,rounds,block_size,index,value):
        ...
        self.shift=[[0,1,8,25],[0,1,8,55],[0,1,16,25]]
        if self.block_size==256:
            self.shift_row=self.shift[0]
        elif self.block_size==384:
            self.shift_row=self.shift[1]
        else:
            self.shift_row=self.shift[2]
    ...
    def gen_constr(self):
        for r in range(self.r):
            ...
            for col in range(sboxnum):
                ...
                y=inv_list(y)
                self.m.update()
                sbox_model.gen_model_from_ine(self.m,x+y,'knot\knot_ine.txt',)

...
class Skinny():
    def __init__(self,rounds,blocksize,flag,index,value):
        ...
        self.a=self.m.addVars([i for i in range(self.r*16*2)],vtype=GRB.BINARY,name='at')

        constr=LinExpr()
        for i in range(self.blocksize*2):
            constr+=self.x[i]
        self.m.addConstr(constr>=15)
                    ...
                    if self.sbox_size==8:
                        for i in range(8):
                            for j in range(2):
                                ...
                                tmp+=[tmps[i][j]]
                                pos=i*2+j
                                sbox_model.gen_model_from_ine(self.m,tmp,'skinny//ine//skinny'+str(pos)+'.txt')
